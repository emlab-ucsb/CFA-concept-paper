...
dataset_filter_10km <- final_eez_dataset %>%
  ...
  group_by(mrgid_eez) %>%
  mutate(distance_to_eez_rounded_km = round(distance_to_eez_boundary_km/10)*10) %>%
  ungroup() %>%
...
dataset_filter_20km <- final_eez_dataset %>%
  ...
  mutate(distance_to_eez_boundary_km = ifelse(inside_eez == FALSE, distance_to_eez_boundary_meters / 1000, -1*(distance_to_eez_boundary_meters / 1000))) %>%
  group_by(mrgid_eez) %>%
  mutate(distance_to_eez_rounded_km = round(distance_to_eez_boundary_km/bin_size_km)*bin_size_km) %>%
  ungroup() %>%
  group_by(mrgid_eez, distance_to_eez_rounded_km) %>%
  summarize(fishing_hours = sum(fishing_hours)) %>%
...
dataset_filter_40km <- final_eez_dataset %>%
  ...
  mutate(distance_to_eez_boundary_km = ifelse(inside_eez == FALSE, distance_to_eez_boundary_meters / 1000, -1*(distance_to_eez_boundary_meters / 1000))) %>%
  group_by(mrgid_eez) %>%
  mutate(distance_to_eez_rounded_km = round(distance_to_eez_boundary_km/40)*40) %>%
  ungroup() %>%
  group_by(mrgid_eez, distance_to_eez_rounded_km) %>%
  summarize(fishing_hours = sum(fishing_hours)) %>%
...
dataset_filter_100km <- final_eez_dataset %>%
  ...
  mutate(distance_to_eez_boundary_km = ifelse(inside_eez == FALSE, distance_to_eez_boundary_meters / 1000, -1*(distance_to_eez_boundary_meters / 1000))) %>%
  group_by(mrgid_eez) %>%
  mutate(distance_to_eez_rounded_km = round(distance_to_eez_boundary_km/100)*100) %>%
  ungroup() %>%
  group_by(mrgid_eez, distance_to_eez_rounded_km) %>%
  summarize(fishing_hours = sum(fishing_hours)) %>%
...
for(i in 1:length(country_names)){
  ...
  ggplot() +
  aes(x = distance_to_eez_rounded_km, y = fishing_hours, group = country_name) +
  geom_point() +
  ...
  ggsave(paste0(results_dir, "/", file_name, ".png"), dpi = 200, width = 12, height = 12)
}
