...
As we are only interested in marine protected areas, we first need to filter this dataset to remove terrestrial and freshwater protected areas (MARINE = 2). Due to the size of the global protected areas dataset, this was done using QGIS prior to importing into R. We then uploaded the resulting shapefile to mapshaper.org and simplified it to 0.5%. The resulting file is stored in `./data/WDPA_Nov2019_Marine_Only/`. 

```{r}
# Read in MPA data and convert to text format for upload to Bigquery
mpa_boundaries <- st_read(here::here("data", "WDPA_Nov2019_Marine_Only/"), layer = "WDPA_Nov2019_Marine_Only", stringsAsFactors=FALSE, as_tibble=TRUE) %>%
  clean_names(case = "snake") %>%
  dplyr::select(name, desig = desig_eng, desig_type, area_km = rep_m_area, no_take, area_no_take_km = no_tk_area, status, year = status_yr, loc_parent_iso3 = parent_iso, loc_iso3 = iso3) %>%
  st_make_valid() %>%
