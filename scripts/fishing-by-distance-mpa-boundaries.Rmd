...
```{r}
# Read in MPA data and do some tidying
mpa_boundaries <- st_read(here::here("data", "WDPA_Nov2019_Marine_Only/"), layer = "WDPA_Nov2019_Marine_Only", stringsAsFactors=FALSE) %>%
  clean_names(case = "snake") %>%
  dplyr::select(name, desig = desig_eng, desig_type, area_km = rep_m_area, no_take, area_no_take_km = no_tk_area, status, year = status_yr, parent_iso3 = parent_iso, iso3 = iso3) %>%
  group_by(name, desig, desig_type, area_km, no_take, area_no_take_km, status, year, parent_iso3, iso3) %>%
